...
public class GraphView extends JComponent {
    ...
    protected void paintComponent(Graphics g) {
        ...
        g2.setColor(new Color(200, 195, 185, 30));
        ...
        // Ink colors for blueprint
        Color inkColor = new Color(20, 15, 10, 180);  // Black ink
        Color stampColor = Theme.ACCENT;  // Red stamp

        // Base map edges as thin ink lines (blueprint style)
        g2.setStroke(new BasicStroke(1.2f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
        g2.setColor(new Color(100, 90, 80, 60));  // Faded ink for base edges
        ...
        // BFS: Ink spreading outward in waves (circular ink blots)
        if ("BFS".equalsIgnoreCase(mode)) {
            ...
            for (int i = 0; i < visCount; i++) {
                ...
                // Ink blot expands like a wave
                double radius = 8 + age * 3 + 2 * Math.sin(pulse + i);
                int alpha = Math.max(40, 120 - age * 15);
                g2.setColor(new Color(inkColor.getRed(), inkColor.getGreen(), inkColor.getBlue(), alpha));
                g2.fill(new Ellipse2D.Double(p[0] - radius, p[1] - radius, radius * 2, radius * 2));
            }
        } else {
            // Dijkstra: Pencil draws edges gradually (handwritten style)
            int visCount = Math.min(visited.size(), visIndex);
            for (int i = 0; i < visCount; i++) {
                String v = visited.get(i);
                double[] p = pos.get(v);
                if (p == null) continue;
                // Subtle pencil mark
                g2.setColor(new Color(inkColor.getRed(), inkColor.getGreen(), inkColor.getBlue(), 80));
                g2.fill(new Ellipse2D.Double(p[0] - 6, p[1] - 6, 12, 12));
            }
        }

        // Path lines - pen-draw animation
        int pathEdges = Math.max(0, path.size() - 1);
        if ("BFS".equalsIgnoreCase(mode)) {
            // BFS: Underline with pen-draw
            g2.setStroke(new BasicStroke(2.5f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
        } else {
            // Dijkstra: Thicker fountain-pen stroke
        ...
        for (int i = 0; i < Math.min(edgeIndex, pathEdges); i++) {
            ...
            // Edge weight (handwritten style for Dijkstra)
            if ("Dijkstra".equalsIgnoreCase(mode) && i < edgeWeights.size()) {
                double midX = (a[0] + b[0]) / 2;
                double midY = (a[1] + b[1]) / 2;
                g2.setColor(new Color(inkColor.getRed(), inkColor.getGreen(), inkColor.getBlue(), 140));
                g2.setFont(getFont().deriveFont(Font.PLAIN, 10f));
                g2.drawString(String.valueOf(edgeWeights.get(i)), (int)midX + 4, (int)midY - 4);
            }
        }
        
        // Drawing current edge
        ...
        // Underline final path (BFS style)
        if ("BFS".equalsIgnoreCase(mode) && edgeIndex >= pathEdges && pathEdges > 0) {
            g2.setStroke(new BasicStroke(1.5f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
            g2.setColor(new Color(inkColor.getRed(), inkColor.getGreen(), inkColor.getBlue(), 180));
            for (int i = 0; i < pathEdges; i++) {
                double[] a = pos.get(path.get(i));
                double[] b = pos.get(path.get(i + 1));
                if (a == null || b == null) continue;
                // Draw underline slightly below the path
                g2.draw(new Line2D.Double(a[0], a[1] + 3, b[0], b[1] + 3));
            }
        }
        
        // Stamp "APPROVED ROUTE" when path is complete
        if (edgeIndex >= pathEdges && pathEdges > 0) {
            double[] last = pos.get(path.get(path.size() - 1));
            if (last != null) {
                int stampW = 140;
                int stampH = 50;
                int stampX = (int)last[0] - stampW / 2;
                int stampY = (int)last[1] - 80;
                
                // Stamp body
                g2.setColor(new Color(stampColor.getRed(), stampColor.getGreen(), stampColor.getBlue(), 200));
                g2.fillRoundRect(stampX, stampY, stampW, stampH, 6, 6);
                g2.setColor(new Color(stampColor.getRed(), stampColor.getGreen(), stampColor.getBlue(), 255));
                g2.setStroke(new BasicStroke(2f));
                g2.drawRoundRect(stampX, stampY, stampW, stampH, 6, 6);
                
                // Stamp text
                g2.setColor(Theme.CARD);
                g2.setFont(getFont().deriveFont(Font.BOLD, 11f));
                String stampText = "APPROVED ROUTE";
                int textW = g2.getFontMetrics().stringWidth(stampText);
                g2.drawString(stampText, stampX + (stampW - textW) / 2, stampY + stampH / 2 + 4);
            }
        }

        // Secondary path (compare mode) - dashed ink
        int path2Edges = Math.max(0, path2.size() - 1);
        if (path2Edges > 0) {
            g2.setStroke(new BasicStroke(2.0f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 10f, new float[] { 6f, 6f }, 0f));
            for (int i = 0; i < path2Edges; i++) {
                double[] a = pos.get(path2.get(i));
                double[] b = pos.get(path2.get(i + 1));
                if (a == null || b == null) continue;
                g2.setColor(new Color(inkColor.getRed(), inkColor.getGreen(), inkColor.getBlue(), 150));
                g2.draw(new Line2D.Double(a[0], a[1], b[0], b[1]));
            }
        }

        // Nodes as stamped location names (blueprint style)
        Font f = getFont().deriveFont(Font.BOLD, 11f);
        g2.setFont(f);
        for (String n : nodes) {
            double[] p = pos.get(n);
            if (p == null) continue;
            boolean inPath = path.contains(n);
            boolean isVisited = visited.contains(n) && visIndex > visited.indexOf(n);

            // Stamped circle (like a rubber stamp)
            if (inPath || isVisited) {
                // Stamped location (red stamp circle)
                g2.setColor(new Color(stampColor.getRed(), stampColor.getGreen(), stampColor.getBlue(), 180));
                g2.fill(new Ellipse2D.Double(p[0] - 16, p[1] - 16, 32, 32));
                g2.setColor(new Color(stampColor.getRed(), stampColor.getGreen(), stampColor.getBlue(), 255));
                g2.setStroke(new BasicStroke(2f));
                g2.draw(new Ellipse2D.Double(p[0] - 16, p[1] - 16, 32, 32));
                
                // "REVIEWED" text on stamp
                if (isVisited && !inPath) {
                    g2.setColor(Theme.CARD);
                    g2.setFont(getFont().deriveFont(Font.BOLD, 8f));
                    String reviewed = "REVIEWED";
                    int rw = g2.getFontMetrics().stringWidth(reviewed);
                    g2.drawString(reviewed, (int)p[0] - rw/2, (int)p[1] + 2);
                }
            } else {
                // Unstamped location (just outline)
                g2.setColor(new Color(inkColor.getRed(), inkColor.getGreen(), inkColor.getBlue(), 100));
                g2.setStroke(new BasicStroke(1.5f));
                g2.draw(new Ellipse2D.Double(p[0] - 14, p[1] - 14, 28, 28));
            }

            // Location name (handwritten style)
            g2.setColor(Theme.TEXT);
            g2.setFont(f);
            int nameW = g2.getFontMetrics().stringWidth(n);
            g2.drawString(n, (int) p[0] - nameW / 2, (int) p[1] - 20);
        }
    ...
    private static void drawCampusEdge(Graphics2D g2, Map<String, double[]> pos, String a, String b, int w) {
        ...
        int alpha = 40 + (int) (40 * t);  // Ink alpha
        float stroke = 0.8f + 0.6f * t;   // Thin ink lines
        g2.setStroke(new BasicStroke(stroke, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
        g2.setColor(new Color(100, 90, 80, alpha));  // Faded ink
